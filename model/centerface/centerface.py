...
class CenterFace(object):
    def __init__(self, landmarks=True):
        ...
        if self.landmarks:
            self.net = cv2.dnn.readNetFromONNX(
                'model/centerface/models/onnx/centerface.onnx')
        else:
            self.net = cv2.dnn.readNetFromONNX(
                'model/centerface/models/onnx/cface.1k.onnx')
    ...
    def postprocess(self, heatmap, lms, offset, scale, threshold):
        if self.landmarks:
            dets, lms = self.decode(heatmap,
                                    scale,
                                    offset,
                                    lms, (self.img_h_new, self.img_w_new),
                                    threshold=threshold)
        else:
            dets = self.decode(heatmap,
                               scale,
                               offset,
                               None, (self.img_h_new, self.img_w_new),
                               threshold=threshold)
        if len(dets) > 0:
            dets[:, 0:4:
                 2], dets[:, 1:4:
                          2] = dets[:, 0:4:
                                    2] / self.scale_w, dets[:, 1:4:
                                                            2] / self.scale_h
            if self.landmarks:
                lms[:, 0:10:
                    2], lms[:, 1:10:
                            2] = lms[:, 0:10:
                                     2] / self.scale_w, lms[:, 1:10:
                                                            2] / self.scale_h
        else:
    ...
    def decode(self, heatmap, scale, offset, landmark, size, threshold=0.1):
        ...
        if len(c0) > 0:
            for i in range(len(c0)):
                s0, s1 = np.exp(scale0[c0[i], c1[i]]) * 4, np.exp(
                    scale1[c0[i], c1[i]]) * 4
                ...
                if self.landmarks:
                    lm = []
                    for j in range(5):
                        lm.append(landmark[0, j * 2 + 1, c0[i], c1[i]] * s1 +
                                  x1)
